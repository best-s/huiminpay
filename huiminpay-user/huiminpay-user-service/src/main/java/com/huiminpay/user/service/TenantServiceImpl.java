package com.huiminpay.user.service;
...
import com.huiminpay.common.cache.util.MD5Util;
import com.huiminpay.common.cache.util.PhoneUtil;
import com.huiminpay.common.cache.util.RandomStringUtil;
import com.huiminpay.user.api.AuthorizationService;
import com.huiminpay.user.api.ResourceService;
import com.huiminpay.user.api.TenantService;
import com.huiminpay.user.api.dto.authorization.AuthorizationInfoDTO;
import com.huiminpay.user.api.dto.authorization.RoleDTO;
import com.huiminpay.user.api.dto.resource.ApplicationDTO;
import com.huiminpay.user.api.dto.tenant.*;
import com.huiminpay.user.convert.*;
import com.huiminpay.user.entity.*;
import com.huiminpay.user.mapper.*;
...
//////////////////////////////////////////租户相关的操作///////////////////////////////////////////////////
    ...
    public TenantDTO createTenantAndAccount(CreateTenantRequestDTO createTenantRequest) {
		...
		if(isExistAccountByMobile(mobile)) {//为true表示 手机号已存在
			...
			if (tenant.getId() == null) {
				throw new BusinessException(CommonErrorCode.E_88888);
			}
			//设置初始化套餐，更新权限
			log.info("初始化套餐");
    ...
    public AccountDTO createAccount(CreateAccountRequestDTO createAccountRequest) {
        ...
        if(isExistAccountByMobile(mobile)){
            throw new BusinessException(CommonErrorCode.E_88888);//手机号在账号中已存在
        }
        //2.校验用户名是否为空及是否存在
