package com.huiminpay.gateway.config;
...
public class ResouceServerConfig {
    ...
    public static final String RESOURCE_ID = "huimin-resource";
    public static final String YH_RESOURCE_ID = "yh-resource";
    ...
    /**
     * 因为腾讯课堂在线 每个微服务都包含有所有接口，不需要登陆的、需要登陆的、需要权限的，不太好在网关层初步鉴别访问权限
     * 所以所有请求都放行， 由后面的微服务去鉴权
     */
    @Configuration
    @EnableResourceServer
    public class YHServiceServerConfig extends ResourceServerConfigurerAdapter {
        ...
        @Override
        public void configure(ResourceServerSecurityConfigurer resources) {
            resources.tokenStore(tokenStore).resourceId(YH_RESOURCE_ID).stateless(true);
            resources.authenticationEntryPoint(point).accessDeniedHandler(handler);
        }
